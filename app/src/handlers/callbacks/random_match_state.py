...
async def handle_random_match_state_callback(callback: CallbackQuery) -> None:
	...
	async with get_session() as session:
		...
		sent = await callback.message.answer(get_searching_message_state(), reply_markup=cancel_kb)
		# Prepare queue row (filter_only_state=True) including message id
		queue_row = ChatQueue(
			user_id=user.id,
			...
			filter_only_state=True,
			filter_only_city=False,
		)
