...
async def handle_search_popular(callback: CallbackQuery) -> None:
	...
	try:
		...
		pass
	text, ok, has_next, has_items = await generate_popular_list(callback.from_user.id if callback.from_user else 0, gender, page=1)
	pag_kb = build_pagination_kb("popular", page=1, gender=gender, has_next=has_next)
