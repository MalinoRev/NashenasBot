...
async def handle_random_match_male_callback(callback: CallbackQuery) -> None:
	...
	async with get_session() as session:
		...
		sent = await callback.message.answer(get_searching_message_male(), reply_markup=cancel_kb)
		# Enqueue with the sent message id (filter_only_boy=True)
		queue_row = ChatQueue(
			user_id=user.id,
