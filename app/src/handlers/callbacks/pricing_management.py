...
async def handle_pricing_management(callback: CallbackQuery) -> None:
	...
	# Check user step and get product values in same session
	async with get_session() as session:
		user: User | None = await session.scalar(select(User).where(User.user_id == user_id))
		...
		if callback_data == "pricing:vip_price":
			...
			print(f"LOG: VIP price step set to: {user.step}")
			...
			await session.commit()
			print(f"LOG: VIP time step set to: {user.step}")
			...
			await session.commit()
			print(f"LOG: Delete price step set to: {user.step}")
			...
			await session.commit()
			print(f"LOG: Unban price step set to: {user.step}")
