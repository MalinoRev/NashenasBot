...
async def handle_support_add_confirm(callback: CallbackQuery) -> None:
	data = callback.data or ""
	if not data.startswith("support_add_confirm:"):
	...
	if data == "support_add_confirm:no":
		# Cancel
		...
		from src.context.messages.replies.support_add_success import get_cancelled_message
	...
	if data.startswith("support_add_confirm:yes:"):
		...
		async with get_session() as session:
			...
			if existing:
				await callback.answer("❌ این کاربر قبلاً پشتیبان است.", show_alert=True)
				return
			# Add
			new_s = Supporter(user_id=target_user.id)
			session.add(new_s)
			await session.commit()
		# Reset step
		async with get_session() as session:
		...
		from src.context.messages.replies.support_add_success import get_success_message, get_success_alert
		try:
