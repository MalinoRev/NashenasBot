...
async def generate_same_age_list(tg_user_id: int, gender: GenderFilter, page: int = 1, page_size: int = 10) -> tuple[str, bool, bool, bool]:
	"""
	Return (text, ok). If ok is False, text contains an error notice.
	Lists up to 10 users whose age is within [me-3, me+3].
	"""
	async with get_session() as session:
		...
		if not my_profile or my_profile.age is None:
			return ("⚠️ ابتدا سن خود را در پروفایل تکمیل کنید.", False, False, False)

		min_age = max(0, int(my_profile.age) - 3)
		max_age = int(my_profile.age) + 3
		...
		lines: list[str] = ["🎯 لیست هم‌سن‌های شما بر اساس آنلاین بودن:", ""]
